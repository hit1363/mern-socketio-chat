...
export const updateProfile = async (req, res) => {
  try {
    ...
    // If already a URL (user re-saving existing image) just return current user
    if (/^https?:\/\//i.test(profilePic)) {
      const existing = await User.findById(userId).select("-password");
      return res.status(200).json(existing);
    }

    // Rough size check (base64 length -> bytes) to prevent huge payloads
    const approxBytes = (profilePic.length * 3) / 4; // base64 size formula
